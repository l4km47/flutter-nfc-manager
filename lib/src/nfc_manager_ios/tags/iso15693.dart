...
class Iso15693IOS {
  ...
  static Iso15693IOS? from(NfcTag tag) {
    ...
    final pigeon = pigeonTag.iso15693;
    return pigeon == null
        ? null
        : Iso15693IOS._(
            pigeonTag.handle!,
  ...
  Future<Uint8List> readSingleBlock({
    ...
    return hostApi.iso15693ReadSingleBlock(
        _handle, requestFlags.map((e) => e.name).toList(), blockNumber);
  }
  ...
  Future<void> writeSingleBlock({
    ...
    return hostApi.iso15693WriteSingleBlock(_handle,
        requestFlags.map((e) => e.name).toList(), blockNumber, dataBlock);
  }
  ...
  Future<List<Uint8List>> readMultipleBlocks({
    ...
    return hostApi
        .iso15693ReadMultipleBlocks(
            _handle,
            requestFlags.map((e) => e.name).toList(),
            blockNumber,
            numberOfBlocks)
  ...
  Future<void> select({
    ...
    return hostApi.iso15693Select(
        _handle, requestFlags.map((e) => e.name).toList());
  }
  ...
  Future<void> writeAfi({
    ...
    return hostApi.iso15693WriteAfi(
        _handle, requestFlags.map((e) => e.name).toList(), afi);
  }
  ...
  Future<void> writeDsfId({
    ...
    return hostApi.iso15693WriteDsfId(
        _handle, requestFlags.map((e) => e.name).toList(), dsfId);
  }
